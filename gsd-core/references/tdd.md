...
4. Persist the RED evidence for every runner: the real command, actual exit status, unmodified output, target test identity, expected result from the plan, and actual result from the captured run.
   ...
   - For supported evidence, use machine validation:
     - Node's built-in test runner: use compatible TAP from the actual command. If the reporter is incompatible, rerun the planned target with Node's compatible TAP reporter, capture that real command and result, then submit that rerun's record to the classifier invocation below and require `RED_EVIDENCE_OK`. A Node run must never fall through to direct inspection.
     - Maven Surefire/Failsafe: capture unchanged XML from `target/surefire-reports/TEST-*.xml` or `target/failsafe-reports/TEST-*.xml` generated by the actual command (#4724). Record the report path, run start, and report modification time; require the report to be newer than the run start and verify the named target class executed in that run. Put that report's original XML in the record's `output`, retain the real command and exit status, and preserve the console output alongside it. Missing, stale, or ambiguous reports require STOP and investigation; old reports must not authorize GREEN. Submit the current target report's record to the classifier invocation below and require `RED_EVIDENCE_OK`. A Maven run must never fall through to direct inspection.
     - For either supported branch, pass the unchanged record to `gsd_run check tdd-red-evidence <record.json> --raw` and require `RED_EVIDENCE_OK` before GREEN. Every existing `INVALID_RED` reason (`unexpected_green`, `zero_tests_discovered`, `nonzero_exit_without_test_failure`, `fixture_or_load_failure`, `no_target_test_failure`, `invalid_record`, `unreadable_record`) trips the gate; classifier failure never authorizes a fallback to direct inspection.
   - For other identified runners without a supported evidence format (for example Vitest), directly inspect the captured output and the test assertion. This branch is self-attestation and has no deterministic backstop: nothing mechanically catches a false claim that the output was inspected and found sound, so the recorded assessment is the artifact a reviewer audits. Known cost of that scope: an unsupported runner whose output happened to be Node-compatible TAP used to receive a machine-checked verdict by accident and now receives direct inspection, accepted so that genuine unsupported-runner RED evidence is no longer rejected (#4692). This fallback excludes Node and Maven Surefire/Failsafe, whose machine validation is mandatory.
...
|------|----------|---------------|------------|
| RED | Yes | `test({phase}-{plan}): ...` | Test exists AND fails before implementation — intentionally: resolve the actual runner first; Node built-in compatible TAP and current Maven Surefire/Failsafe XML must return `RED_EVIDENCE_OK`, other identified runners without supported evidence require direct inspection, unresolved runners stop, and all evidence receives semantic inspection of the planned target assertion |
...
2. **INVALID_RED in RED phase (#3770):** A nonzero exit is not RED by itself. Apply step 4's shared evidence contract and resolve the actual runner before choosing a branch. An unresolved runner stops for investigation. Node's built-in test runner with compatible TAP and Maven Surefire/Failsafe with current XML reports use step 4's classifier invocation on the unchanged record; an `INVALID_RED` verdict halts GREEN. Follow step 4's Node rerun and Maven report-freshness requirements; neither branch may fall back to direct inspection. Other identified runners without supported evidence receive direct inspection, and every branch receives semantic inspection of the planned target assertion. Any invalid, incomplete, ambiguous, or contradictory evidence stops GREEN; do not fabricate a parser verdict or Node counters.
