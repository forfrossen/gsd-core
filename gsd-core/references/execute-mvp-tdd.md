...
2. **The test was actually red — INTENTIONALLY (#3770).** For every runner, persist the real command, actual exit status, unmodified output, target test identity, expected result from the plan's `<behavior>`, and actual result from the captured run. Identify the runner from the actual command that produced the evidence, including in mixed-runner projects. If the runner is unresolved, STOP and investigate before selecting a validation branch. Do not infer Node from `package.json` or package metadata, npm/pnpm or another package manager, or TAP-shaped text alone.
