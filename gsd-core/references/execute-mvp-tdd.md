...
2. **The test was actually red — INTENTIONALLY (#3770).** For every runner, persist the real command, actual exit status, unmodified output, target test identity, expected result from the plan's `<behavior>`, and actual result from the captured run. Identify the runner from the actual command that produced the evidence, including in mixed-runner projects. If the runner is unresolved, STOP and investigate before selecting a validation branch.
   - If the actual command invokes Node's built-in test runner and the output is compatible TAP, pass the unchanged record to `gsd_run check tdd-red-evidence <record.json> --raw` and require `RED_EVIDENCE_OK`. If the reporter is incompatible, rerun the planned target with Node's compatible TAP reporter, capture that real command and result, then submit that rerun's record to the same classifier invocation above and require `RED_EVIDENCE_OK`. A Node run stays on this branch; it must never fall through to direct inspection. Do not infer Node from `package.json` or package metadata, npm/pnpm or another package manager, or TAP-shaped text alone. Every existing `INVALID_RED` reason (`unexpected_green`, `zero_tests_discovered`, `nonzero_exit_without_test_failure`, `fixture_or_load_failure`, `no_target_test_failure`, `invalid_record`, `unreadable_record`) trips the gate.
   - For every other identified runner, directly inspect the captured output and the test assertion. This branch is self-attestation and has no deterministic backstop: nothing mechanically catches a false claim that the output was inspected and found sound, so the recorded assessment is the artifact a reviewer audits. Known cost of that scope: a non-Node runner whose output happened to be Node-compatible TAP used to receive a machine-checked verdict by accident and now receives direct inspection unconditionally, accepted so that genuine non-Node RED evidence is no longer rejected (#4692).
